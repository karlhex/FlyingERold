        ...
        <div class="=my-2 overfow-x-auto sm:-mx-6 lg:-mx-8">
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <table class="min-w-full divide-y divide-gray-200 border" id="mytable">
                    <thead>
                        <tr>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __("options.key")  }}</th>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __("options.option")  }}</th>
                            <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">{{ __("options.value")  }}</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr class="hover:bg-green-200">
                            <td class="px-4 bg-gray-50 "><input type="text" class="bg-gray-100 font-medium  leading-4 text-gray-500 border-2 rounded" wire:model.defer='filters.key' /> </td>
                            <td class="px-4 bg-gray-50 "><input type="text" class="bg-gray-100 font-medium  leading-4 text-gray-500 border-2 rounded" wire:model.defer='filters.options' /> </td>
                            <td class="px-4 bg-gray-50 "><input type="text" class="bg-gray-100 font-medium  leading-4 text-gray-500 border-2 rounded" wire:model.defer='filters.value' /> </td>
                        ...
                        @foreach ($data as $key => $item)
                            <tr class="hover:bg-green-200">
                                <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->key  }}</td>
                                <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->option  }}</td>
                                <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->value  }}</td>
